...
func (r *ResponseBuilder) Content(msg string) *ResponseBuilder {
	r.content = &msg
	return r
}
...
// Send sends a new message (defers automatically if not already)
func (r *ResponseBuilder) Send() error {
	if r.ephemeral {
		...
		if err := r.e.DeferCreateMessage(false); err != nil {
			return err
		}
	}
